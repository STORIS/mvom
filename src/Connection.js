...
class Connection {
	...
	constructor({ connectionManagerUri, account, logger, cacheMaxAge, timeout }) {
		...
		Object.defineProperties(this, {
			 ...
			 */
			status: {
				value: connectionStatusConstants.DISCONNECTED,
				enumerable: true,
				writable: true,
			},
			/**
	...
	open = async () => {
		...
		this.status = connectionStatusConstants.CONNECTING;
		...
		if (this._serverFeatureSet.invalidFeatures.length > 0) {
			...
			this.status = connectionStatusConstants.DISCONNECTED;
		...
		this.status = connectionStatusConstants.CONNECTED;
	};
	 ...
	 */
	model = (schema, file) => {
		if (this.status !== connectionStatusConstants.CONNECTED) {
