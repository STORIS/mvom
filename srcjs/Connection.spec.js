	...
	describe('constructor', () => {
		...
		test('should set instance variables based on constructor parameters', () => {
			expect(
				...
				logger: mockLogger,
				status: connectionStatusConstants.DISCONNECTED,
				_cacheMaxAge: 'baz',
				_timeout: 'qux',
			});
		});
	...
	describe('instance methods', () => {
		...
		describe('open', () => {
			...
			test('should change state to CONNECTED if nothing invalid is found', async () => {
				...
				expect(connection.status).toBe(connectionStatusConstants.CONNECTED);
			});
			...
			test('should leave state as DISCONNECTED if something invalid is found', async () => {
				...
				expect(connection.status).toBe(connectionStatusConstants.DISCONNECTED);
			});
		});
		...
		describe('model', () => {
			...
			beforeEach(() => {
				connection.status = connectionStatusConstants.DISCONNECTED;
			...
			test('should return the result from compileModel', () => {
				connection.status = connectionStatusConstants.CONNECTED;
		...
		describe('_getDbServerInfo', () => {
			...
			test('should call moment once with the epoch and once without anything', async () => {
				await connection._getDbServerInfo();
				expect(moment.calledTwice).toBe(true);
				expect(moment.calledWith(epoch)).toBe(true);
